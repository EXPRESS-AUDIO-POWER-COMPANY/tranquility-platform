...
export function BookingEstimator() {
    ...
    try {
      ...
      return {
        ...
        error: error instanceof Error
          ? error.message
          : 'Enter valid property details to continue.',
      }
    }
  }, [
    serviceType,
  ...
  const conditionReviewRecommended =
    propertyCondition !== 'routine' || selectedAddOns.some((addOn) => addOn.requiresReview)
    ...
    <div className="grid gap-8 lg:grid-cols-[1.25fr_0.75fr]">
      <div className="rounded-[2.2rem] bg-white p-6 shadow-soft sm:p-8">
          ...
          {steps.map((label, index) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[10px] font-bold uppercase tracking-[0.13em] ${active ? 'text-tranquility-charcoal' : 'text-black/45'}`}>
        ...
        {step === 1 ? (
          <section className="pt-7">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-black/55">
              Choose the residential service, frequency, size, and general condition. Your property profile updates as you go.
            </p>
            <div className="mt-8 grid gap-6 sm:grid-cols-2">
              <label className="grid gap-2 text-sm font-semibold">
                Service type
                <select className="field" onChange={(event) => { setServiceType(event.target.value as ServiceType); resetReview() }} value={serviceType}>
              ...
              <label className="grid gap-2 text-sm font-semibold">
                Frequency
                <select className="field" onChange={(event) => { setFrequency(event.target.value as Frequency); resetReview() }} value={frequency}>
              ...
              <label className="grid gap-2 text-sm font-semibold sm:col-span-2">
                ...
                <span className="text-xs font-normal text-black/50">
                  Condition helps identify when a closer property review may be useful before pricing or scheduling is finalized.
                </span>
              </label>
            </div>
          </section>
            ...
            {otherRooms > 0 ? (
              <label className="mt-6 grid gap-2 text-sm font-semibold">
                ...
                <textarea
                  ...
                  onChange={(event) => { setOtherSpaceDetails(event.target.value); resetReview() }}
                  placeholder="Example: game room, office, sunroom, loft, mud room."
                  value={otherSpaceDetails}
                />
          ...
          <section className="pt-7">
            ...
            <div className="mt-8 rounded-2xl border border-black/10 p-5">
              ...
              {petsPresent ? (
                <label className="mt-4 grid gap-2 text-sm font-semibold">
                  ...
                  <textarea
                    className="field min-h-24"
                    onChange={(event) => { setPetDetails(event.target.value); resetReview() }}
                      ...
                      <span className="text-black/55">
                        {livePricing
                          ...
                          : addOn.requiresReview
                            ...
                            : 'Available with selected service'}
                      </span>
                    </span>
                  </label>
                ))}
            ...
            <label className="mt-7 grid gap-2 text-sm font-semibold">
              ...
              <textarea
                ...
                maxLength={1200}
                onChange={(event) => { setServiceNotes(event.target.value); resetReview() }}
                placeholder="Example: Please focus extra attention on the kitchen and guest bathroom."
                value={serviceNotes}
              />
          ...
          <section className="pt-7">
            ...
            <div className="mt-8 grid gap-5 sm:grid-cols-2">
              ...
              <label className="grid gap-2 text-sm font-semibold">
                ...
                {postalCode && !postalCodeValid ? <span className="text-xs font-normal text-red-700">Enter a valid ZIP code.</span> : null}
              </label>
              <label className="grid gap-2 text-sm font-semibold">
            ...
            <label className="mt-6 grid gap-2 text-sm font-semibold">
              ...
              <textarea
                className="field min-h-24"
                maxLength={800}
                onChange={(event) => { setAccessInstructions(event.target.value); resetReview() }}
                placeholder="Gate instructions, parking notes, front desk, call-on-arrival preference, or other useful access information."
            ...
            {estimate?.requiresManualQuote ? (
              ...
              <div className="mt-8">
                <Button disabled={!contactComplete || Boolean(estimateState.error)} onClick={() => setReadyForReview(true)} type="button">
                  Review service request <ArrowRight className="ml-2 size-4" />
                </Button>
            ...
            {readyForReview && estimate ? (
              <div className="mt-8 rounded-[1.8rem] border border-tranquility-sage/30 bg-tranquility-sage/10 p-6" role="status">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm leading-6 text-black/58">
                      Contact Tranquility to confirm availability and finalize the reservation. {livePricing ? 'The planning estimate remains subject to the actual property condition and requested scope.' : 'Pricing is confirmed after the property scope is reviewed.'}
                    </p>
                  </div>
                </div>
                <dl className="mt-6 grid gap-3 border-t border-black/8 pt-5 text-sm sm:grid-cols-2">
                  ...
                  <div><dt className="text-black/45">Pricing</dt><dd className="mt-1 font-semibold">{livePricing ? `$${estimate.total} planning estimate` : 'Confirm after review'}</dd></div>
                ...
                <div className="mt-6 flex flex-wrap gap-3 border-t border-black/8 pt-5">
                  <a className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-tranquility-charcoal px-5 text-sm font-semibold text-white" href={siteConfig.phoneHref}>
                    <Phone className="size-4" aria-hidden="true" />Call {siteConfig.shortName}
                  </a>
                  <a className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-black/10 bg-white px-5 text-sm font-semibold" href={emailRequestHref}>
                    <Mail className="size-4" aria-hidden="true" />Email request
                  </a>
                </div>
              </div>
            ) : null}
        ...
        <div className="mt-9 flex flex-wrap items-center justify-between gap-3 border-t border-black/7 pt-6">
          <Button disabled={step === 1} onClick={() => goToStep(step - 1)} type="button" variant="ghost">
            <ArrowLeft className="mr-2 size-4" /> Previous
          </Button>
          {step < 4 ? (
            <Button disabled={!canContinue()} onClick={() => goToStep(step + 1)} type="button">
      ...
      <aside aria-live="polite" className="h-fit rounded-[2.2rem] bg-tranquility-charcoal p-7 text-white shadow-soft lg:sticky lg:top-32 sm:p-8">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-white/55">
          {livePricing ? 'Your cleaning estimate' : 'Your service profile'}
        </p>
        ...
        {estimateState.error ? (
          ...
          <>
            ...
            <p className="mt-4 text-sm leading-6 text-white/70">This property falls outside the instant-pricing range. A virtual consultation gives Tranquility room to review the space accurately.</p>
            <ButtonLink className="mt-7 w-full bg-white text-tranquility-charcoal hover:bg-tranquility-ivory" to="/quote">
              Start custom quote <ArrowRight className="ml-2 size-4" />
            </ButtonLink>
          </>
        ) : estimate ? (
          <>
            {livePricing ? (
              <>
                ...
                <p className="mt-2 text-xs font-semibold uppercase tracking-[0.14em] text-white/50">Current planning estimate</p>
              ...
              <>
                <p className="mt-5 font-serif text-4xl leading-tight">Pricing confirmed after review.</p>
                <p className="mt-4 text-sm leading-7 text-white/68">Your selected service and property details are used to prepare the request without publishing draft or unapproved pricing.</p>
                <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.055] p-4 text-sm text-white/70">
                  <p className="font-semibold text-white">{serviceLabel(serviceType)} · {frequencyLabel(frequency)}</p>
                  <p className="mt-2 text-xs leading-6 text-white/55">Final pricing is confirmed before the reservation is finalized.</p>
                </div>
              </>
            )}

            <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.055] p-4 text-xs leading-6 text-white/65">
            ...
            {conditionReviewRecommended ? (
              <div className="mt-4 flex items-start gap-2 rounded-2xl border border-tranquility-stone/25 bg-tranquility-stone/10 p-4 text-xs leading-6 text-white/70">
            ...
            <ButtonLink className="mt-6 w-full bg-white text-tranquility-charcoal hover:bg-tranquility-ivory" to="/quote" variant="secondary">
        ...
        <div className="mt-6 border-t border-white/10 pt-5">
          <p className="flex items-start gap-2 text-xs leading-6 text-white/55">
            ...
            {livePricing
              ? 'This is a planning estimate based on the details selected. Property condition, requested scope, and specialty items can change the final service price.'
              : 'Draft pricing is not displayed. Tranquility confirms the service scope and pricing before a reservation is finalized.'}
          </p>
        </div>
      </aside>
    </div>
