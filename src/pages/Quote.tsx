...
function getInitialQuoteService() {
  ...
  if (requested === 'standard') return 'standard-cleaning'
...
export function Quote() {
  ...
  const [propertyType, setPropertyType] = useState(() => getInitialQuoteService() === 'commercial' ? 'commercial' : 'residential')
  const [serviceInterest, setServiceInterest] = useState(getInitialQuoteService)
        ...
        <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-start">
          <aside className="rounded-[2.2rem] bg-tranquility-charcoal p-8 text-white shadow-soft lg:sticky lg:top-32 sm:p-10">
            ...
            <div className="mt-8 grid gap-4 border-t border-white/10 pt-7">
              {['Residential custom quotes', 'Larger homes and unusual layouts', 'Commercial cleaning inquiries', 'Special conditions or detailed scope'].map((item) => <span key={item} className="flex items-start gap-3 text-sm text-white/78"><CheckCircle2 className="mt-0.5 size-4 shrink-0 text-tranquility-stone" aria-hidden="true" />{item}</span>)}
            </div>
            <div className="mt-8 rounded-2xl border border-white/10 bg-white/[0.055] p-5"><ShieldCheck className="size-5 text-tranquility-stone" aria-hidden="true" /><p className="mt-3 text-xs leading-6 text-white/60">Property photos should show only the cleaning scope. Avoid identification documents, payment information, security credentials, or unrelated sensitive material.</p></div>
          </aside>

          <form className="rounded-[2.2rem] bg-white p-7 shadow-soft sm:p-10" onSubmit={handleSubmit}>
            <div><p className="eyebrow">Property details</p><h2 className="mt-4 font-serif text-3xl">Tell Tranquility what needs attention.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-black/55">Provide enough detail to understand the property, desired service, current condition, and timing before a custom price is prepared.</p></div>
            ...
            <label className="mt-7 grid gap-2 text-sm font-semibold">Tell us about the space<textarea className="field min-h-36" maxLength={2000} onChange={(event) => { setDetails(event.target.value); resetReview() }} placeholder="Describe the rooms, current condition, special concerns, pets, access details, stains, appliances, flooring, or the cleaning outcome you want." required value={details} /><span className="text-right text-xs font-normal text-black/45">{details.length}/2000</span></label>

            <div className="mt-7 rounded-[1.8rem] border border-dashed border-black/18 bg-tranquility-ivory p-6 sm:p-7">
              <Camera className="size-6 text-tranquility-moss" aria-hidden="true" /><p className="mt-4 font-semibold">Add interior property photos</p><p className="mt-2 text-sm leading-6 text-black/55">Choose up to {MAX_FILES} JPG, PNG, WebP, HEIC, or HEIF images, maximum 8 MB each and 50 MB total.</p>
              ...
              {files.length ? <><div className="mt-5 flex items-center justify-between gap-4 text-xs text-black/50"><span>{files.length} of {MAX_FILES} images selected</span><span>{formatBytes(totalUploadSize)} total</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{previews.map(({ file, url }) => <div key={`${file.name}-${file.lastModified}`} className="overflow-hidden rounded-2xl border border-black/8 bg-white">{url ? <img alt={`Selected property preview: ${file.name}`} className="h-36 w-full object-cover" src={url} /> : null}<div className="flex items-center gap-3 p-3 text-xs text-black/65"><FileImage className="size-4 shrink-0 text-tranquility-moss" aria-hidden="true" /><span className="min-w-0 flex-1 truncate">{file.name}</span><button aria-label={`Remove ${file.name}`} className="grid size-8 shrink-0 place-items-center rounded-full hover:bg-tranquility-ivory" onClick={() => removeFile(file)} type="button"><Trash2 className="size-4" aria-hidden="true" /></button></div></div>)}</div></> : null}
            </div>
            ...
            {readyForReview ? <div className="mt-6 rounded-[1.8rem] border border-tranquility-sage/30 bg-tranquility-sage/10 p-6" role="status"><div className="flex items-start gap-3"><CheckCircle2 className="mt-1 size-5 shrink-0 text-tranquility-moss" aria-hidden="true" /><div><p className="font-serif text-2xl">Your quote request details are ready.</p><p className="mt-2 text-sm leading-6 text-black/60">Contact Tranquility to continue the review. If you selected property photos, ask for the preferred secure way to send them with the request.</p></div></div><dl className="mt-6 grid gap-3 border-t border-black/8 pt-5 text-sm sm:grid-cols-2"><div><dt className="text-black/45">Customer</dt><dd className="mt-1 font-semibold">{fullName}</dd></div><div><dt className="text-black/45">Property</dt><dd className="mt-1 font-semibold">{propertyType} • {city}, {postalCode}</dd></div><div><dt className="text-black/45">Service</dt><dd className="mt-1 font-semibold">{serviceInterest.replaceAll('-', ' ')}</dd></div><div><dt className="text-black/45">Timing</dt><dd className="mt-1 font-semibold">{preferredTiming}</dd></div><div><dt className="text-black/45">Condition</dt><dd className="mt-1 font-semibold">{condition.replaceAll('-', ' ')}</dd></div><div><dt className="text-black/45">Photos</dt><dd className="mt-1 font-semibold">{files.length ? `${files.length} selected` : 'No photos selected'}</dd></div></dl><div className="mt-6 flex flex-wrap gap-3 border-t border-black/8 pt-5"><a className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-tranquility-charcoal px-5 text-sm font-semibold text-white" href={siteConfig.phoneHref}><Phone className="size-4" />Call {siteConfig.shortName}</a><a className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-black/10 bg-white px-5 text-sm font-semibold" href={`mailto:${siteConfig.email}?subject=${emailSubject}&body=${emailBody}`}><Mail className="size-4" />Email quote details</a></div></div> : null}
          </form>
        </div>
      </div>
    </section>
